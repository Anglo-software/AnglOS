 ...
 * This is because each structure table is aligned on a page and one page in size
 * And as such the least significant bits 0-11 are just all 0's.
 * 
 * Page Map Level 4 Entry:
 * |0|00000000000|0000000000000000000000000000000000000000|0000|00000000| AVL = 0x001
 * |X|[   AVL   ]|[               PDPT  PA               ]|[AV]|[ FLAG ]|
 * |D|62       52|51                                    12|11 8|7      0|
 * 
 * Page Directory Pointer Table Entry:
 * |0|00000000000|0000000000000000000000000000000000000000|0000|00000000| AVL = 0x002
 * |X|[   AVL   ]|[                PDT PA                ]|[AV]|[ FLAG ]|
 * |D|62       52|51                                    12|11 8|7      0|
 * 
 * Page Directory Entry:
 * |0|00000000000|0000000000000000000000000000000000000000|0000|00000000| AVL = 0x004
 * |X|[   AVL   ]|[                PT  PA                ]|[AV]|[ FLAG ]|
 * |D|62       52|51                                    12|11 8|7      0|
 * 
 ...
 * 
 * Page Table Entry:
 * |0|0000|0000000|0000000000000000000000000000000000000000|000|000000000| AVL = 0x008
 ...
 *      -> P   = Present
 * 
 * PAT -> If PAT is supported, then PAT along with PCD and PWT shall indicate the memory caching type, otherwise it is reserved and must be set to 0.
 * 
 * G   -> Tells the processor not to invalidate the TLB entry correstponding to the page upon a MOV to CR3 instruction. 
 *        Bit 7 (PGE) in CR4 must be set to enable global pages.
 * 
 * D   -> Used to determine whether a page has been written to.
 * 
 * A   -> Used to discover whether a table entry was read during a VA translation. 
 *        If it has, then the bit is set, otherwise, it is not. 
 *        The bit will not be cleared by the CPU, that burden falls on the OS (if it needs this bit at all).
 * 
 * PCD -> If the bit is set, the page will not be cached, otherwise, it will be.
 * 
 * PWT -> If the bit is set, write-through caching is enabled, otherwise write-back is enabled instead.
 * 
 ...
 * 
 * R/W -> If the bit is set, the page is read/write, otherwise the page is read-only. 
 *        The WP bit in CR0 determines if this is only applied to userland, always giving the kernel write access.
 * 
 * P   -> If this bit is set, the page is actually in physical memory at the moment.
 * 
 * Page fault error codes pushed onto stack: (these bits are the only ones used, all others are reserved)
 ...
 *        --- could also allocate a lot of pages at once and keep a counter of how many are used
 *        --- allocate more if they are all used up
 * free   - free one table (page) using its physical address
 * add    - create an entry at an index in a page structure pointing to a physical address with flags
 * update - update an entry at an index in a page structure pointing to a physical address with flags
 * remove - remove an entry at an index in a page structure and free its children if it is not a PTE
 * getpfv - do a page walk on a virtual address
 * updcr3 - update the CR3 register with a physical address to the P4T
 * vmaloc - allocate some number of contiguous pages in virtual memory starting at vptr with flags
 * vfree  - free some number of contiguous pages in virtual memory starting at vptr
 */
void* alloc_table();
