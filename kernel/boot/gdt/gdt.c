...
void init_gdt(uint64_t num_cpu) {
    spin_lock(&lock);
    gdtr.limit = (sizeof(gdt_desc_t) * GDT_MAX_DESCRIPTORS) - 1;
    gdtr.base = (uintptr_t)&gdt[num_cpu][0];

    gdt_add_descriptor(num_cpu, 0, 0,       0,                                                                     0);
    gdt_add_descriptor(num_cpu, 0, 0xFFFF,  GDT_BASIC_DESCRIPTOR | GDT_DESCRIPTOR_EXECUTABLE,                      0);
    gdt_add_descriptor(num_cpu, 0, 0xFFFF,  GDT_BASIC_DESCRIPTOR,                                                  0);
    gdt_add_descriptor(num_cpu, 0, 0xFFFFF, GDT_BASIC_DESCRIPTOR | GDT_DESCRIPTOR_EXECUTABLE,                      GDT_GRANULARITY_4K | GDT_GRANULARITY_X32);
    gdt_add_descriptor(num_cpu, 0, 0xFFFFF, GDT_BASIC_DESCRIPTOR,                                                  GDT_GRANULARITY_4K | GDT_GRANULARITY_X32);
    gdt_add_descriptor(num_cpu, 0, 0,       GDT_BASIC_DESCRIPTOR | GDT_DESCRIPTOR_EXECUTABLE,                      GDT_GRANULARITY_X64);
    gdt_add_descriptor(num_cpu, 0, 0,       GDT_BASIC_DESCRIPTOR,                                                  0);
    gdt_add_descriptor(num_cpu, 0, 0,       GDT_BASIC_DESCRIPTOR | GDT_DESCRIPTOR_DPL,                             0);
    gdt_add_descriptor(num_cpu, 0, 0,       GDT_BASIC_DESCRIPTOR | GDT_DESCRIPTOR_DPL | GDT_DESCRIPTOR_EXECUTABLE, GDT_GRANULARITY_X64);
...
void gdt_add_descriptor(uint64_t num_cpu, uint64_t base, uint32_t limit, uint8_t access, uint8_t granularity) {
    ...
    gdt[num_cpu][gindex[num_cpu]].granularity = granularity | ((limit >> 16) & 0x0F);
