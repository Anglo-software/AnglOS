...
/* Auxiliary data for vsnkprintf_helper(). */
struct vsnkprintf_aux {
    char* p;        /* Current output position. */
    int length;     /* Length of output string. */
    int max_length; /* Max length of output string. */
};

static void vsnkprintf_helper(char, void*);

/* Like vkprintf(), except that output is stored into BUFFER,
...
int vsnkprintf(char* buffer, size_t buf_size, const char* format, va_list args)
{
    ...
    /* Do most of the work. */
    __vkprintf(format, args, vsnkprintf_helper, &aux);
...
/* Like kprintf(), except that output is stored into BUFFER,
   which must have space for BUF_SIZE characters.  Writes at most
   BUF_SIZE - 1 characters to BUFFER, followed by a null
   terminator.  BUFFER will always be null-terminated unless
   BUF_SIZE is zero.  Returns the number of characters that would
...
int snkprintf(char* buffer, size_t buf_size, const char* format, ...)
{
    ...
    retval = vsnkprintf(buffer, buf_size, format, args);
    va_end(args);
...
int kprintf(const char* format, ...)
{
    ...
    va_start(args, format);
    retval = vkprintf(format, args);
    va_end(args);

    return retval;
}

/* kprintf() formatting internals. */

/* A kprintf() conversion. */
struct kprintf_conversion {
    /* Flags. */
    enum {
        MINUS = 1 << 0, /* '-' */
        PLUS  = 1 << 1, /* '+' */
                ...
                void* aux)
{
    for (; *format != '\0'; format++) {
        ...
        case 'i': {
            ...
            switch (c.type) {
            case CHAR: value = (signed char)va_arg(args, int); break;
            case SHORT: value = (short)va_arg(args, int); break;
            case INT: value = va_arg(args, int); break;
            case INTMAX: value = va_arg(args, intmax_t); break;
            case LONG: value = va_arg(args, long); break;
            case LONGLONG: value = va_arg(args, long long); break;
            case PTRDIFFT: value = va_arg(args, ptrdiff_t); break;
            case SIZET:
            ...
            format_integer(value < 0 ? -value : value, true, value < 0, &base_d,
                           &c, output, aux);
        } break;
        ...
        case 'X': {
            ...
            switch (c.type) {
            case CHAR: value = (unsigned char)va_arg(args, unsigned); break;
            case SHORT: value = (unsigned short)va_arg(args, unsigned); break;
            case INT: value = va_arg(args, unsigned); break;
            case INTMAX: value = va_arg(args, uintmax_t); break;
            case LONG: value = va_arg(args, unsigned long); break;
            case LONGLONG: value = va_arg(args, unsigned long long); break;
            case PTRDIFFT: value = va_arg(args, ptrdiff_t);
#if UINTMAX_MAX != PTRDIFF_MAX
                value &= ((uintmax_t)PTRDIFF_MAX << 1) | 1;
#endif
                break;
            case SIZET: value = va_arg(args, size_t); break;
            default: NOT_REACHED();
            }
        ...
        case 'n':
            ...
            __kprintf("<<no %%%c in kernel>>", output, aux, *format);
            break;

        default:
            __kprintf("<<no %%%c conversion>>", output, aux, *format);
            break;
        }
    }
}
                                    ...
                                    struct kprintf_conversion* c, va_list* args)
{
    ...
    for (;;) {
        switch (*format++) {
        case '-': c->flags |= MINUS; break;
        case '+': c->flags |= PLUS; break;
        case ' ': c->flags |= SPACE; break;
        case '#': c->flags |= POUND; break;
        case '0': c->flags |= ZERO; break;
        case '\'': c->flags |= GROUP; break;
        default: format--; goto not_a_flag;
        }
    }
not_a_flag:
    if (c->flags & MINUS)
    ...
    case 't': c->type = PTRDIFFT; break;

    case 'z': c->type = SIZET; break;
...
void __kprintf(const char* format, void (*output)(char, void*), void* aux, ...)
{
    ...
    va_start(args, aux);
    __vkprintf(format, args, output, aux);
    va_end(args);
}
...
void hex_dump(uintptr_t ofs, const void* buf_, size_t size, bool ascii)
{
    ...
    while (size > 0) {
        ...
        /* Print line. */
        kprintf("%08jx  ", (uintmax_t)ROUND_DOWN(ofs, per_line));
        for (i = 0; i < start; i++)
            kprintf("   ");
        for (; i < end; i++)
            kprintf("%02hhx%c", buf[i - start],
                    i == per_line / 2 - 1 ? '-' : ' ');
        if (ascii) {
            for (; i < per_line; i++)
                kprintf("   ");
            ...
            for (i = 0; i < start; i++)
                kprintf(" ");
            for (; i < end; i++)
                kprintf("%c", isprint(buf[i - start]) ? buf[i - start] : '.');
            for (; i < per_line; i++)
                kprintf(" ");
            kprintf("|");
        }
